...
#define Y           1     // Y axis
#define Z           2     // Z axis
#define MPU_ADDRESS 0x69  // I2C address of the MPU-6050
...
char auth[] = "c6CfljdZdOEmMGBa7BsEqiHuJRwG_0rr";
char ssid[] = "VIVEK";
char pass[] = "9840271370";
boolean armed = false;
int ppm[8];
WidgetLCD lcd(V1);
int fm=1;
...
BLYNK_WRITE(V2) {  
  ppm[2] = param.asInt(); 
  pulse_length[0] = ppm[2];
  Serial.print("V2: "); Serial.print( ppm[2]);
}

BLYNK_WRITE(V3) {
  ppm[3] = param.asInt(); 
  pulse_length[1] = ppm[3]; 
  Serial.print("V3: "); Serial.print( ppm[3]);
}

BLYNK_WRITE(V6){
  ppm[1] = param.asInt(); // assigning incoming value from pin V6 to a variable
  ppm[0]=1500;  
  pulse_length[2] = ppm[1];
  pulse_length[3] = ppm[0];
}
...
BLYNK_WRITE(V5){
  ...
  Serial.print("V5: "); Serial.print( ppm[7]);
}
...
void MPU6050_Init(){
  delay(150);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_SMPLRT_DIV, 0x07);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_PWR_MGMT_1, 0x01);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_PWR_MGMT_2, 0x00);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_CONFIG, 0x00);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_GYRO_CONFIG, 0x00);//set +/-250 degree/second full scale
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_ACCEL_CONFIG, 0x00);// set +/- 2g full scale
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_FIFO_EN, 0x00);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_INT_ENABLE, 0x01);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_SIGNAL_PATH_RESET, 0x00);
  I2C_Write(MPU_ADDRESS, MPU6050_REGISTER_USER_CTRL, 0x00);
}
...
void applyMotorSpeed() {
    ...
    //while (PORTD >= 16) {
    while(digitalRead(D5) == LOW && digitalRead(D6) == LOW && digitalRead(D7) == LOW && digitalRead(D8) == LOW){      
        now        = micros();
        difference = now - loop_timer;
        if (difference >= pulse_length_esc1) digitalWrite(14, LOW); //PORTD &= B11101111; // Set pin #4 LOW
        if (difference >= pulse_length_esc2) digitalWrite(12, LOW);//PORTD &= B11011111; // Set pin #5 LOW
        if (difference >= pulse_length_esc3) digitalWrite(13, LOW); //PORTD &= B10111111; // Set pin #6 LOW
        if (difference >= pulse_length_esc4) digitalWrite(15, LOW); //PORTD &= B01111111; // Set pin #7 LOW
    }
}
...
bool isStarted() {
    ...
    Serial.print("status: "); Serial.print(status); Serial.print(pulse_length[mode_mapping[YAW]]);
